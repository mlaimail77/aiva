...
def _compile_safe_disable(fn):
    # These helpers are reused across multiple channel widths, which causes
    # Dynamo to repeatedly retrace them inside compiled VAE decode paths.
...
class RMS_norm(nn.Module):
    ...
    def forward(self, x):
        ...
        normalized = normalized.mul(self.scale).to(dtype=x.dtype)
        gamma = self.gamma.to(dtype=x.dtype)
        bias = self.bias.to(dtype=x.dtype) if isinstance(self.bias, torch.Tensor) else self.bias
